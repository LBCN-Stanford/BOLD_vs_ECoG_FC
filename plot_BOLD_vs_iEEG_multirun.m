...
xlabel('ECoG Run');
print('-opengl','-r300','-dpng',strcat([pwd,filesep,'BOLD_vs_ECoG_multirun']));
end
pause; close;

%plot partial corr (distance-corrected)
FigHandle = figure('Position', [400, 600, 400, 700]);
figure(1)
for i=1:length(allsubs_seed_HFB_medium_allruns)
...
xlabel('ECoG Run');
print('-opengl','-r300','-dpng',strcat([pwd,filesep,'BOLD_vs_ECoG_multirun_partialcorr']));
end
pause; close;
