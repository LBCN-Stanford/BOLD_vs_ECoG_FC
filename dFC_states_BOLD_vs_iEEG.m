...
% absolute
if states==2
    if k1k2BOLD_vs_k1k2iEEG_change_corr<0
       BOLD_change=-BOLD_k1_minus_k2;
    else
        BOLD_change=BOLD_k1_minus_k2;
    end
        
    scatter(BOLD_change,iEEG_k1_minus_k2,'MarkerEdgeColor','k','MarkerFaceColor',[0 0 0]); 
h=lsline; set(h(1),'color',[0 0 0],'LineWidth',3);
set(gca,'Fontsize',14,'FontWeight','bold','LineWidth',2,'TickDir','out');
set(gcf,'color','w');
title({['kstate 1 vs 2: ' Window_dur ' sec windows']; ...
    ['r = ' num2str(abs(k1k2BOLD_vs_k1k2iEEG_change_corr)) ' p = ' num2str(k1k2BOLD_vs_k1k2iEEG_change_p)]},'Fontsize',12); 
xlabel('BOLD FC change');
ylabel('iEEG FC change');
set(gcf,'PaperPositionMode','auto');
    print('-opengl','-r300','-dpng',strcat([pwd,filesep,'electrode_spheres/SBCA',filesep,'figs',filesep,'kstates',filesep,[roi1 '_k2_BOLD_vs_iEEG_change_corr_' Window_dur]]));
    pause; close;
end
...
if states==2    
    FigHandle = figure(1);
set(FigHandle,'Position',[50, 50, 700, 300]);
subplot(2,1,1)
plot(IDX_BOLD,'Color',[.4 .4 .4],'LineWidth',2);
set(gca,'Fontsize',12,'FontWeight','bold','LineWidth',2,'TickDir','out');
set(gca,'box','off'); 
set(gcf,'color','w');
set(gca,'Ytick',1:2)
title(['BOLD states: ' Window_dur ' sec Windows']);
xlim([0 length(IDX_BOLD)]);
ylim([0.5 2.5])
ylabel('State'); xlabel('Window number');

subplot(2,1,2)
plot(IDX_iEEG,'Color',[.4 .4 .4],'LineWidth',2);
set(gca,'Fontsize',12,'FontWeight','bold','LineWidth',2,'TickDir','out');
set(gca,'box','off'); 
set(gcf,'color','w');
set(gca,'Ytick',1:2)
title(['iEEG states: ' Window_dur ' sec Windows']);
ylim([0.5 2.5]);
xlim([0 length(IDX_iEEG)]);
ylabel('State'); xlabel('Window number');
print('-opengl','-r300','-dpng',strcat([pwd,filesep,'electrode_spheres/SBCA',filesep,'figs',filesep,'kstates',filesep,[roi1 '_kstates_ts_' Window_dur '_k' num2str(states)]]));
pause; close;
end
...
% correspondence
if states==2
   maxcorr=abs(k1k2BOLD_vs_k1k2iEEG_change_corr); 
...
% BOLD change plot (for state pair with max similarity to an iEEG state
% change)
kstates=num2str(states);
...
cfg.elecColorScale='minmax';
cfgOut=plotPialSurf(Patient,cfg);
print('-opengl','-r300','-dpng',strcat([pwd,filesep,'electrode_spheres/SBCA',filesep,'figs',filesep,'kstates',filesep,[roi1 '_' BOLD_title '_' Window_dur '_k' kstates]]));

% iEEG change plot (for state pair with max similarity to a BOLD state
% change)
cfg=[];
...
cfg.elecColorScale='minmax';
cfgOut=plotPialSurf(Patient,cfg);
print('-opengl','-r300','-dpng',strcat([pwd,filesep,'electrode_spheres/SBCA',filesep,'figs',filesep,'kstates',filesep,[roi1 '_' iEEG_title '_' Window_dur '_k' kstates]]));
pause; close;